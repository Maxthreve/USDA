...
require 'pry'
count = 0
Zip::File.open("sr26.zip") do |zipfile|
  #food desc
  #extract the file from the zip file

  zipfile.get_entry("FOOD_DES.txt").extract
  File.foreach("FOOD_DES.txt","r") do |line|
    l = line.split("^")
    n =[]
    l.each do |i|
      temp = i.gsub(/~/, "").gsub(/\n/,"").rstrip()
...
#############
  ...
  File.foreach("WEIGHT.txt") do |line|
    l = line.ssplit("^")
    n = []
    l.each do |i|
      temp = i.gsub(/~/,"").gsub(/\n/, "").rstrip()
...
# Langual file
#############
  zipfile.get_entry("LANGUAL.txt").exract
...
##########################
  ...
  zipfile.get_entry("LANGDESC.txt")
...
#######################
  zipfile.get_entry("NUT_DEF.txt").extract
  File.foreach("NUT_DEF.txt") do |line|
    l = line.split("^")
    n = []
    l.each do |i|
      temp = i.gsub(/~/, "").gsub(/\n/, "").rstrip()
...
#################
  ...
  File.foreach("SRC_CD.txt") do |ine|
   l = line.split("^")
   n = []
   l.each do |i|
     temp = i.gsub(/~/, "").gsub(/\n/, "").rstrip()
...
#################
  ...
  zipfile.get_entry("DATASRCLN.txt").extract
    File.foreach("DATASRCLN.txt") do |line|
      l = line.split("^")
      n = []
      l.each do |i|
        temp = i.gsub(/~/, "").gsub(/\n/, "").rstrip()
